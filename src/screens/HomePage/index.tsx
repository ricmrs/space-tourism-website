import Box from "@/components/Box";
import Button from "@/components/Button";
import Text from "@/components/Text";
import Head from "next/head";

export default function HomePage() {
  return (
    <>
      <Head><title>Space Tourism</title></Head>
      <Box tag="main" styleSheet={{
        backgroundImage: {
          xs: "url('assets/home/background-home-mobile.jpg')",
          sm: "url('assets/home/background-home-tablet.jpg')",
          lg: "url('assets/home/background-home-desktop.jpg')",
          xl: "url('assets/home/background-home-desktop.jpg')"
        },
        backgroundSize: 'cover',
        backgroundRepeat: 'no-repeat',
        backgroundPosition: 'center',
        width: '100%',
        minHeight: '100%',
        alignItems: 'center',
        paddingBottom: '81px',
        paddingRight: '24px',
        flexDirection: { xs: 'column', sm: 'column', lg: 'row', xl: 'row' },
        paddingLeft: { xs: '24px', sm: '24px', lg: '125px', xl: '165px' },
        paddingTop: { xs: '112px', sm: '202px', lg: '280px', xl: '340px' },
        gap: { xs: '80px', sm: '80px', lg: '10vw', xl: '20vw' }
      }}>
        <Box styleSheet={{
          width: { xs: '325px', sm: '450px', lg: '450px' },
          alignItems: { xs: 'center', sm: 'center', lg: 'flex-start' },
          textAlign: 'center',
        }}>
          <Text tag="h2" variant="heading5" colorVariant="secondary">So, you want to travel to</Text>
          <Text tag="h1" variant="heading1">Space</Text>
          <Text variant="body" colorVariant="secondary" styleSheet={{ textAlign: { xs: 'center', lg: 'justify' } }}>
            Let’s face it; if you want to go to space, you might as well genuinely go to outer space and not hover kind of on the edge of it. Well sit back, and relax because we’ll give you a truly out of this world experience!
          </Text>
        </Box>
        <Button
          type="rounded"
          colorVariant="tertiary"
          enablePulseEffect={true}
          styleSheet={{
            alignSelf: 'center'
          }}
          href='/destination'
        >
          <Text colorVariant="primary" variant="heading4">Explore</Text>
        </Button>
      </Box>
    </>
  )
}
